...
import modTgcItems from "../database/modTGC_items.json";
import modTgcClothes from "../database/modTGC_clothes.json";
import globals from "../database/globals.json";
import traderAssort from "../database/traders/668aaff35fd574b6dcc4a686/assort.json";
import traderSuits from "../database/traders/668aaff35fd574b6dcc4a686/suits.json";
...
class TGCItems implements IPostDBLoadMod {
    ...
    public async postDBLoad(container: DependencyContainer): Promise<void> {
        ...
        //Item Filters
        if (this.mydb.modTgcItems && typeof this.mydb.modTgcItems === "object" && Object.keys(this.mydb.modTgcItems).length > 0) {
            for (const modTGCID in this.mydb.modTgcItems) this.addToFilters(modTGCID);
        }
    ...
    private copyToFilters(itemClone: string, modTGCID: string): void {
        ...
        for (const item in this.db.templates.items) {
            if (item in this.mydb.modTgcItems) continue;
    ...
    private addLocales(modTGCID: string, modTGCItem?: ImodTGCItem, modTGCArticle?: ICustomizationItem): void {
        ...
        for (const localeID in this.db.locales.global) //For each possible locale/language in SPT's database
        {
            ...
            if (isItem && "locales" in modTGCEntry) {
                if (localeID in (modTGCEntry as ImodTGCItem).locales) //If the language is entered in modTgcItems, use that
                {
                    localeEntry = {
